...
export class BookingsRepository {
    ...
    private normalizeBookingCode(bookingCode: string): string {
        return bookingCode.trim().toUpperCase().replace(/-/g, '');
    }
    ...
    async findById(id: string): Promise<NullableType<Booking>> {
        const entity = await this.bookingRepo.findOne({
            where: { bookingId: id },
            relations: ['trip', 'trip.route', 'trip.busCompany', 'trip.tripStops', 'trip.tripStops.stop', 'trip.tripStops.stop.station'],
        });
    ...
    async findEntityById(id: string): Promise<NullableType<BookingEntity>> {
        return this.bookingRepo.findOne({
            where: { bookingId: id },
            relations: ['trip', 'trip.busCompany', 'trip.tripStops', 'trip.tripStops.stop', 'trip.tripStops.stop.station'],
        });
    }
    ...
    async findByCode(bookingCode: string): Promise<NullableType<Booking>> {
        ...
        let entity = await this.bookingRepo.findOne({ where: { bookingCode: bookingCode.trim() } });
        if (!entity) {
            entity = await this.bookingRepo
                .createQueryBuilder('booking')
                .where(`REPLACE(UPPER(booking.bookingCode), '-', '') = :normalizedCode`, { normalizedCode })
                .getOne();
        }
        if (!entity) return null;
    ...
    async findEntityByCode(bookingCode: string): Promise<NullableType<BookingEntity>> {
        const normalizedCode = this.normalizeBookingCode(bookingCode);
        const exact = await this.bookingRepo.findOne({
    ...
    async create(userId: string | null, dto: CreateBookingDto, companyId?: string): Promise<Booking> {
        return this.dataSource.transaction(async (manager) => {
            ...
            const { seats, totalAmount, expiresAt, status } = await this.resolveBookingDraft(manager, dto, companyId);
    ...
    private async resolveBookingDraft(
        ...
        seats: Array<{ seatId: string; price: number }>
        totalAmount: number
        expiresAt?: Date
        ...
        return { seats, totalAmount, expiresAt, status };
    }

    async quote(dto: CreateBookingDto, companyId?: string): Promise<{ totalAmount: number; expiresAt?: Date }> {
        return this.dataSource.transaction(async (manager) => {
            const draft = await this.resolveBookingDraft(manager, dto, companyId);
            return {
